<template>
  <div class="auth-card">
    ...
    <p class="subtitle">We'll generate a reset link for this admin account</p>
    ...
    <div v-else class="sent-state">
      ...
      <p>{{ message }}</p>
      <p class="hint">No email delivery is configured for admin resets yet — ask whoever has server log access to retrieve the link, or a Super Admin can reset your password directly from Sub-Admins.</p>
    </div>

    <p class="switch"><NuxtLink to="/admin/login">← Back to admin sign in</NuxtLink></p>
  </div>
</template>
...
async function onSubmit() {
  ...
  loading.value = true
  try {
    const res = await $fetch('/api/admin/auth/forgot-password', { method: 'POST', body: { email: email.value } })
