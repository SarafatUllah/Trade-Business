...
async function onSubmit() {
  ...
  error.value = ''
  try {
    await $fetch('/api/admin/auth/reset-password', { method: 'POST', body: { token, newPassword: password.value } })
    done.value = true
  } catch (e: any) {
    error.value = e?.data?.statusMessage || 'Could not reset the password. The link may have expired.'
  } finally {
    loading.value = false
  }
}
